...
async function callOpenRouterAPI(input, messages, systemPrompt, keys, setStatusMessage, signal, modelId) {
    ...
    // モデルリスト: 指定あり→そのモデルのみ、なし→動的無料リストをコーディング適性順で使用
    let modelList;
    if (modelId) {
        modelList = [{ id: modelId, label: modelId }];
    } else {
        // 最初のキーでモデル一覧を取得（キャッシュあり）
...
async function callAI(input, messages, systemPrompt, apiKeys, setStatusMessage, signal, selectedModel) {
    ...
    if (selectedModel && selectedModel !== 'auto') {
        ...
        if (prefix === 'or' && orKeys.length > 0) {
            // auto_free は動的無料リストで呼ぶ（modelIdをnullにして動的取得させる）
            const resolvedModelId = modelId === 'auto_free' ? null : modelId;
            return await callOpenRouterAPI(input, messages, systemPrompt, orKeys, setStatusMessage, signal, resolvedModelId);
        }
