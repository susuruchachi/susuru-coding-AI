    ...
    useEffect(() => {
        ...
        if (auth) {
            const unsubscribe = auth.onAuthStateChanged(async (fbUser) => {
                ...
                if (fbUser) {
                    // Firebase上のAPIキー設定を読み込む
                    const db = window.getFirebaseDb();
                    let savedApiKeys = { gemini: '', openai: '', anthropic: '', openrouter: '' };
                    if (db) {
                        try {
                            const snap = await db.ref(`users/${fbUser.uid}/settings/apiKeys`).once('value');
                            if (snap.exists()) savedApiKeys = snap.val();
                        } catch (e) {
                    ...
                    window._currentApiKeys = savedApiKeys;
                    setUser(userData);
                } else {
                    // 未ログイン: ローカルモード用のみlocalStorageを参照
...
function LoginScreen({ onLogin }) {
    ...
    const handleGoogleLogin = async () => {
        try {
            ...
            // onAuthStateChanged が自動的にユーザーをセットするので、ここでは何もしない
            // （APIキーは onAuthStateChanged 内で Firebase から読み込まれる）
        } catch (error) {
    ...
    const handleLocalLogin = () => {
        ...
        window._currentApiKeys = cleanedKeys;
        onLogin(userData);
    };
    ...
    return (
        <div className="login-screen">
            ...
            <div className="login-subtitle">v0.06.27 - 動的無料モデル・バージョン保存修正</div>
...
function CoderApp({ user, onLogout, firebaseUser }) {
    ...
    const [streamingText, setStreamingText] = useState(''); // ストリーミング中の生成テキスト
    const [remoteStreamingText, setRemoteStreamingText] = useState(''); // 別デバイスからのストリーミング表示用
    ...
    // ─── Firebase同期の設計 ────────────────────────────────────
    // projectsMap: Firebase上の最新データ（onValueで更新）
    // currentProject: ローカルで編集中のデータ（AIの書き込みが反映される）
    // 書き込みuseEffect → Firebase書き込み → onValue発火 → projectsMap更新のみ
    // currentProjectはonValueでは上書きしない（編集中データを守る）
    const firebaseProjectsRef = useRef(null); // 書き込み中のプロジェクトIDを追跡
    const pendingWriteRef = useRef({}); // { [projectId]: timestamp } 書き込み中フラグ
    const prevProjectRef = useRef(null); // 差分同期用の前回状態
    const isLoadingRef = useRef(false); // isLoading のリアルタイム参照（onValueクロージャ用）

    // isLoadingRefをisLoadingと常に同期（onValueクロージャ内で最新値を参照するため）
    useEffect(() => {
        isLoadingRef.current = isLoading;
    }, [isLoading]);

    // Firebase /streaming/{projectId} を監視して別デバイスの生成中テキストをリアルタイム表示
    useEffect(() => {
        if (!user.isFirebase || !currentProject?.id) return;
        const db = window.getFirebaseDb();
        if (!db) return;
        const streamRef = db.ref(`users/${user.id}/streaming/${currentProject.id}`);
        const handler = streamRef.on('value', snap => {
            if (isLoadingRef.current) return; // 自分が生成中なら無視（自分のはstreamingTextで表示）
            const val = snap.val();
            setRemoteStreamingText(val?.text || '');
        });
        return () => streamRef.off('value', handler);
    }, [user.isFirebase, user.id, currentProject?.id]);

    // ページ読み込み後: pendingRequestがあれば自動再送（再読込対応）
    const resumeCheckedRef = useRef(false);
    useEffect(() => {
        resumeCheckedRef.current = false; // プロジェクト切り替えのたびにリセット
    }, [currentProject?.id]);

    useEffect(() => {
        if (!user.isFirebase || !currentProject?.id || isLoading) return;
        if (resumeCheckedRef.current) return;
        resumeCheckedRef.current = true;
        const db = window.getFirebaseDb();
        if (!db) return;
        db.ref(`users/${user.id}/pendingRequest/${currentProject.id}`).once('value').then(snap => {
            if (!snap.exists()) return;
            const { msg, selectedModel: savedModel, ts } = snap.val();
            // 5分以上古いpendingは無視（古すぎる）
            if (Date.now() - ts > 5 * 60 * 1000) {
                snap.ref.remove();
                return;
            }
            // 最後のメッセージがすでにassistantなら完了済み→削除だけ
            ...
            console.log('🔄 未完了リクエストを再送:', msg);
            setStatusMessage('🔄 再読込前の生成を再開中...');
            // selectedModelを一時的に上書きして再送
            sendMessage(msg, true);
        }).catch(() => {});
    }, [user.isFirebase, user.id, currentProject?.id, isLoading]);

    useEffect(() => {
        // ローカルモード: localStorageから読み込み
        if (!user.isFirebase) {
            ...
            if (saved) {
                try {
                    ...
                    if (projArray.length > 0) setCurrentProject(projArray[0]);
                } catch (e) {
        ...
        userProjectsRef.on('value', (snapshot) => {
            ...
            // currentProjectは「自分が書き込み中でない」かつ「未選択」の場合のみ更新
            setCurrentProject(prev => {
                ...
                // ② 書き込み中フラグが立っているプロジェクトは上書きしない（書き込み後5秒間）
                const isPending = pendingWriteRef.current[prev.id]
                    && Date.now() - pendingWriteRef.current[prev.id] < 5000;
                if (isPending) {
                    return prev;
                }
                const fbVersion = projArray.find(p => p.id === prev.id);
                if (!fbVersion) return prev;
                // ③ ローカルの方がメッセージが多い（Firebase未反映の新しいデータがある）場合は上書きしない
                const localMsgCount = (prev.messages || []).length;
                const fbMsgCount = (fbVersion.messages || []).length;
                if (localMsgCount > fbMsgCount) {
                    return prev;
                }
                // ④ ローカルのファイルの方がバージョンの合計が高い場合は上書きしない
                const localFileVer = (prev.files || []).reduce((sum, f) => sum + (f.version || 1), 0);
                const fbFileVer = (fbVersion.files || []).reduce((sum, f) => sum + (f.version || 1), 0);
                if (localFileVer > fbFileVer) {
                    return prev;
                }
                // 別デバイスからの更新を反映
                return fbVersion;
    ...
    useEffect(() => {
        ...
        if (!user.isFirebase) {
            setProjects(prev => {
                const updated = prev.map(p => p.id === currentProject.id ? currentProject : p);
        ...
        // 書き込み開始: このプロジェクトのonValueコールバックで上書きされないようフラグ
        pendingWriteRef.current[currentProject.id] = Date.now();
        const projectIdAtWrite = currentProject.id;
        // 書き込み完了後にフラグをクリア（onValueが先に発火してもフラグで保護される）
        const clearPending = () => {
            setTimeout(() => {
                delete pendingWriteRef.current[projectIdAtWrite];
            }, 2000);
        };

        if (!prev || prev.id !== currentProject.id) {
            // プロジェクト切り替え時: メタのみ書き込み
            const meta = {
            ...
            db.ref(base + '/meta').set(meta)
                .then(clearPending)
                .catch(e => { console.error('Firebase meta sync:', e); clearPending(); });
        } else {
            // 差分のみ書き込み
            const writes = [];
            if (JSON.stringify(prev.messages) !== JSON.stringify(currentProject.messages)) {
                writes.push(db.ref(base + '/messages').set(currentProject.messages || []).catch(e => console.error('Firebase messages sync:', e)));
            }
            if (JSON.stringify(prev.files) !== JSON.stringify(currentProject.files)) {
                writes.push(db.ref(base + '/files').set(currentProject.files || []).catch(e => console.error('Firebase files sync:', e)));
            }
            ...
            if (writes.length > 0) {
                Promise.all(writes).then(clearPending).catch(clearPending);
            } else {
                clearPending();
            }
        }
    ...
    const createProject = () => {
        ...
        if (name) {
            ...
            if (user.isFirebase) {
                // Firebaseに直接書き込む（onValueで自動反映されるのでsetProjectsは不要）
                const db = window.getFirebaseDb();
                if (db) {
                    const base = `users/${user.id}/projects/${proj.id}`;
                    const meta = { id: proj.id, name: proj.name, readme: '', changelog: [], createdAt: proj.createdAt };
                    Promise.all([
                        db.ref(base + '/meta').set(meta),
                        db.ref(base + '/files').set([]),
                        db.ref(base + '/messages').set([])
                    ]).then(() => {
                        setCurrentProject(proj);
                    }).catch(e => console.error('Firebase createProject:', e));
                }
            } else {
    ...
    const sendMessage = async (message = null, _isResume = false) => {
...
[/FILE]`;
            ...
            // APIキーを取得: window._currentApiKeys（最新）とuser.apiKeysをマージ
            const _wk = window._currentApiKeys || {};
            const _uk = user.apiKeys || {};
            // 複数キー対応: 配列または文字列を結合してユニーク配列にする
            const mergeKeys = (k) => {
                const vals = [_wk[k], _uk[k]].flat().filter(v => v && typeof v === 'string' && v.trim());
                return [...new Set(vals)];
            };
            const apiKeys = {
            ...
            // 再読込対応: このリクエストをFirebaseに保存（再読込後に自動再送できるよう）
            if (user.isFirebase && !_isResume) {
                const db = window.getFirebaseDb();
                if (db) db.ref(`users/${user.id}/pendingRequest/${updatedProj.id}`).set({
                    msg,
            ...
            // Firebase streaming書き込み用（throttle: 500ms）
            let streamingWriteTimer = null;
            const writeStreamingToFirebase = (text) => {
                if (!user.isFirebase) return;
                if (streamingWriteTimer) return; // 既にタイマー待機中はスキップ
                streamingWriteTimer = setTimeout(() => {
                    streamingWriteTimer = null;
                    const db = window.getFirebaseDb();
                    if (db) db.ref(`users/${user.id}/streaming/${updatedProj.id}`).set({ text, ts: Date.now() }).catch(() => {});
                }, 500);
            };
            const clearStreamingFromFirebase = () => {
                if (streamingWriteTimer) { clearTimeout(streamingWriteTimer); streamingWriteTimer = null; }
                if (!user.isFirebase) return;
                const db = window.getFirebaseDb();
                if (db) db.ref(`users/${user.id}/streaming/${updatedProj.id}`).remove().catch(() => {});
            };

            // ストリーミング用: 生成中テキストをstateで管理してリアルタイム表示
            const onChunk = (partialText) => {
                setStreamingText(partialText);
                writeStreamingToFirebase(partialText);
            };

            const result = await window.callAI(msg, updatedProj.messages || [], systemPrompt, apiKeys, setStatusMessage, abortCtrl.signal, selectedModel, onChunk);
            setStreamingText(''); // ストリーミング完了後クリア
            clearStreamingFromFirebase();
            ...
            setStreamingText('');
            clearStreamingFromFirebase();
            // pendingRequestを削除（完了・エラー・中断いずれも）
            if (user.isFirebase) {
                const db = window.getFirebaseDb();
                if (db) db.ref(`users/${user.id}/pendingRequest/${currentProject?.id}`).remove().catch(() => {});
            }
            abortControllerRef.current = null;
        }
    };
...
コードの修正版は出力しないでください（指摘のみ）。`;
        ...
        try {
            // APIキーを取得（sendMessageと同じロジック）
            const _wk2 = window._currentApiKeys || {};
            const _uk2 = user.apiKeys || {};
            const mergeKeys2 = (k) => {
                const vals = [_wk2[k], _uk2[k]].flat().filter(v => v && typeof v === 'string' && v.trim());
                return [...new Set(vals)];
            };
            const apiKeys = {
    ...
    const reviewProject = async () => {
        ...
        try {
            const _wk = window._currentApiKeys || {};
            const mk = (k) => { const v = [_wk[k], user.apiKeys?.[k]].flat().filter(x => x && typeof x === 'string' && x.trim()); return [...new Set(v)]; };
    ...
    const updateFile = (content) => {
        ...
        const updated = currentProject.files.map(f =>
            f.id === currentFile.id
                ? { ...f, content, version: (f.version || 1) + 0.001 } // 手動編集はminorバージョン（.001刻み）
                : f
        );
                        ...
                        <div className="messages" ref={messagesContainerRef}>
                            ...
                            {(currentProject.messages || []).map((msg, i) => (
                                <div key={i}>
                                    ...
                                    <div style={{display:'flex',gap:'8px',alignItems:'center',flexWrap:'wrap'}}>
                                        ...
                                        >📋 コピー</button>
                                        {msg.role === 'assistant' && (() => {
                                            // [FILE:...]ブロックとコードブロック(```)を抽出
                                            const parts = [];
                                            // [FILE: name]...[/FILE] 形式
                                            const fileRe = /\[FILE:[^\]]*\]([\s\S]*?)\[\/FILE\]/g;
                                            let m;
                                            while ((m = fileRe.exec(msg.content)) !== null) parts.push(m[1].trim());
                                            // ```lang ... ``` 形式（[FILE]未使用の場合）
                                            if (parts.length === 0) {
                                                const codeRe = /```[^\n]*\n([\s\S]*?)```/g;
                                                while ((m = codeRe.exec(msg.content)) !== null) parts.push(m[1].trim());
                                            }
                            ...
                            {isLoading && (
                                <div className="message">
                                    <div className="message-content assistant" style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                                        {streamingText ? (
                                            <div style={{whiteSpace:'pre-wrap',wordBreak:'break-word'}}>
                                                {streamingText}
                                                <span style={{display:'inline-block',width:'8px',height:'14px',background:'#58a6ff',marginLeft:'2px',verticalAlign:'text-bottom',animation:'blink 0.8s step-end infinite'}}></span>
                                            </div>
                                        ) : (
                            ...
                            {!isLoading && remoteStreamingText && (
                                <div className="message">
                                    <div className="message-content assistant" style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                                        ...
                                        <div style={{fontSize:'11px',color:'#8b949e'}}>📡 別デバイスで生成中...</div>
                                    </div>
                                </div>
                            )}
                ...
                <SettingsModal 
                    ...
                    onUpdateApiKeys={(newKeys) => {
                        ...
                        if (user.isFirebase) {
                            const db = window.getFirebaseDb();
                            if (db) {
                                db.ref(`users/${user.id}/settings/apiKeys`).set(newKeys)
                                    .catch(e => console.error('APIキー保存失敗:', e));
                            }
                        } else {
